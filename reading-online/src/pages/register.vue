    ...
    <div class="register">
        ...
        <van-form ref="registerform" @submit="onSubmit" validate-first @failed="onFailed">
			...
			<van-field
                ...
                label="昵称"
                clearable
                placeholder="中文或字母开头，可以有数字或下划线，1-10位"
...
export default {
	...
	data() {		
		return {
			// 验证密码 以字母开头 只能包含字母、数字
			reg_pass: /^[a-zA-Z]\w{6,15}$/,
			// 验证手机号 以1开头 11位
			reg_tel: /^[1]\d{10}$/,
			// 验证用户名  可以由字母、数字、下划线和中文组成，以中文或字母开头，1-10位
			reg_name: /^([a-zA-Z]|[\u4e00-\u9fa5]){1}([a-zA-Z0-9]|[\u4E00-\u9FA5]|[_]){0,10}$/,
			ruleForm: {
				user_id: 0,
				user_tel: '',
				user_name: '',
